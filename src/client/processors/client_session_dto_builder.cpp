...
ClientSessionDtoBuilder::ClientSessionDtoBuilder(ClientSession &session) : _session(session) {}

MessageDTO ClientSessionDtoBuilder::fillOneMessageDTOFromCl(const std::shared_ptr<Message> &message, std::size_t chat_id) {
...
std::optional<ChatDTO> ClientSessionDtoBuilder::fillChatDTOQt(const std::shared_ptr<Chat> &chat_ptr) {
  ChatDTO chat_dto;
  ...
  for (const auto &participant : participants) {
    ...
    if (user_ptr) {
      ...
      participants_dto.lastReadMessage = chat_ptr->getLastReadMessageId(user_ptr);

      participants_dto.deletedMessageIds.clear();
      const auto &deleted_messages_map = chat_ptr->getDeletedMessagesMap();
      const auto deleted_messages_it = deleted_messages_map.find(participants_dto.login);
