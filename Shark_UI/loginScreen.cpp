...
void loginScreen::setDatabase(std::shared_ptr<Qt_Session> sessionPtr)
{
...
{
  auto result = _sessionPtr->checkLoginPsswordQt(ui->loginEdit->text().toStdString(), ui->passwordEdit->text().toStdString());

  if (!result) {
    QMessageBox::critical(this, tr("Error"),
                          tr("Login or Password is wrong"));
    return;

  }
  else {
    QMessageBox::critical(this, tr("Sucsess"),
                          tr("ok!!!"));
...
void loginScreen::on_loginButtonBox_rejected()
{
